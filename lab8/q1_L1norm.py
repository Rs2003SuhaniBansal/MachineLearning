...
"""Adding l1 norm in the cost function"""
def cost_func(X, y, theta, l):
    cost = (1/2) * np.sum((hypothesis(X, theta) - y)  ** 2) + l*(np.sum(theta[1:]))
    return cost

"""adding l1 norm while computing the derivative or the gradient"""
def comp_deriv(X, y, theta, l):
    deriv = X.T.dot(hypothesis(X, theta) - y)

    # L1 gradient is the sign of the weights
    l1_grad = l * np.sign(theta)
    l1_grad[0] = 0  # Don't regularize bias

    return deriv + l1_grad
